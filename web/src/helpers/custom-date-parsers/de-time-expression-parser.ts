/* eslint-disable */
import {ParsedComponents, ParsedResult, Parser} from 'chrono-node';
...
export default function DETimeExpressionParser() {
	// @ts-expect-error
	...
	this.extract = function (text, ref, match) {
		...
		if (meridiem >= 0) {
			...
			var startAtPM =
				result.start.isCertain('meridiem') && result.start.get('meridiem') == 1;
